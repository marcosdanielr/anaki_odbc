...
mod tests {
    ...
    fn test_create_connection() {
        ...
        let free_result = odbc_free_connection(handle);
        assert_eq!(
            free_result,
    ...
    fn test_connect_success() {
        ...
        let connect_result = odbc_connect(handle, conn_url_c.as_ptr());
    ...
    fn test_null_handle() {
        ...
        let result = odbc_connect(std::ptr::null_mut(), conn_url_c.as_ptr());
        assert_eq!(
            result,
            OdbcError::InvalidHandle as c_int,
            "Should return InvalidHandle error for null handle"
        );
    }
    ...
    fn test_null_connection_string() {
        ...
        let result = odbc_connect(handle, std::ptr::null());
        assert_eq!(
            result,
